...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE: // 窗口创建时候的消息.
        ...
        AppendMenu(hmenu, MF_STRING, IDR_START, "像素替换"); // 为菜单添加选项
        AppendMenu(hmenu, MF_STRING, IDR_TANK, "幻影坦克");
        // AppendMenu(hmenu, MF_POPUP | MF_STRING, IDR_DLC, L"整蛊模式");
        AppendMenu(hmenu, MF_STRING, IDR_COCKROACH, "绝世大蟑螂");
        //=======================================================================================
        ...
        {
        case OUTPUT_JPG:
            AppendMenu(hmenu, MF_STRING, IDR_OUTPUT_MODE, "输出格式：JPEG");
            break;
        case OUTPUT_PNG:
            AppendMenu(hmenu, MF_STRING, IDR_OUTPUT_MODE, "输出格式：PNG");
            break;
        default:
            break;
        }
        //=======================================================================================
        AppendMenu(hmenu, MF_SEPARATOR, NULL, NULL);
        AppendMenu(hmenu, MF_STRING, IDR_PAUSE, "停止混淆");
        //=======================================================================================
        AppendMenu(hmenu, MF_SEPARATOR, NULL, NULL);
        AppendMenu(hmenu, MF_STRING, IDR_LOADCFG, "重载混淆参数");
        //=======================================================================================
        AppendMenu(hmenu, MF_SEPARATOR, NULL, NULL);
        AppendMenu(hmenu, MF_STRING, IDR_ABOUT, "关于");
        AppendMenu(hmenu, MF_STRING, IDR_EXIT, "退出");

        // AppendMenu(hsubmode, MF_STRING, IDR_COCKROACH, L"绝世大蟑螂");
        break;
    ...
    case WM_USER: // 连续使用该程序时候的消息.
        if (lParam == WM_RBUTTONDOWN)
        {
            ...
            if (id == IDR_TANK)
            {
                ...
                ModifyMenu(hmenu, 7, MF_STRING | MF_BYPOSITION, IDR_OUTPUT_MODE, "输出格式：PNG");
                LOG("Enable mirage tank mode");
            }
            if (id == IDR_LOADCFG)
            {
                loadConfig();
                MessageBox(hwnd, TEXT("混淆参数已更新"), szAppClassName, MB_OK);
            }
            if (id == IDR_DLC)
            {
                // GetCursorPos(&pt);//取鼠标坐标
            ...
            if (id == IDR_ROTATE)
            {
                ...
                {
                case 0:
                    ModifyMenu(hmenu, 4, MF_STRING | MF_BYPOSITION, IDR_ROTATE, "顺时针旋转 0°");
                    break;
                case 90:
                    ModifyMenu(hmenu, 4, MF_STRING | MF_BYPOSITION, IDR_ROTATE, "顺时针旋转 90°");
                    break;
                case 180:
                    ModifyMenu(hmenu, 4, MF_STRING | MF_BYPOSITION, IDR_ROTATE, "顺时针旋转 180°");
                    break;
                case 270:
            ...
            if (id == IDR_OUTPUT_MODE)
            {
                if (MIX_MODE != TANK_MODE)
                {
                    ...
                    {
                    case OUTPUT_JPG:
                        ModifyMenu(hmenu, 7, MF_STRING | MF_BYPOSITION, IDR_OUTPUT_MODE, "输出格式：JPEG");
                        break;
                    case OUTPUT_PNG:
                        ModifyMenu(hmenu, 7, MF_STRING | MF_BYPOSITION, IDR_OUTPUT_MODE, "输出格式：PNG");
                        break;
                ...
                {
                    MessageBox(hwnd, TEXT("幻影坦克模式无法使用PNG以外格式输出"), szAppClassName, MB_OK);
                }
            }
            if (id == IDR_ABOUT)
                MessageBox(hwnd, TEXT("自动混淆剪贴板中的图片\n请注意gif会被修改为静态图片"), szAppClassName, MB_OK);
            if (id == IDR_EXIT)
            {
